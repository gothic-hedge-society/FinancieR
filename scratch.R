...
start_date <- end_date - 365*3                                 # 3-year window
end_date
start_date
...
# If shorting is allowed, then there are twice as many "assets" in play because
# each asset can be shorted. We'll use the prefix "s_" to denote a short.

# # The returns we expect for shorting an asset are the negative values of the
# # returns we should expect from buying that asset long:
# exp_rtn <- c(
#   exp_rtn, stats::setNames(exp_rtn * -1, paste0("s_", names(exp_rtn)))
# )
# 
# # Volatility stays the same for shorts:
